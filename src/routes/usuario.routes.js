...
router.post("/usuario/vendedor", async function(req, res){
    ...
    if (!id || !usuarioController.checaGerente(id)) return res.status(403).send("Acesso negado");
...
router.get("/usuario/:id", async function(req, res){
    let id = await usuarioController.authToken(req);
    if (!id || !usuarioController.checaVendedor(id)) return res.status(403).send("Acesso negado");
